...
const addBranchController = async (req, res, next) => {
  ...
  try {
    ...
    // use instance + save so unit tests mocking Branch.prototype.save observe the call
    const branchInstance = new Branch(req.body);
    const newBranch = await branchInstance.save();
...
module.exports = {
  ...
  // compatibility aliases expected by unit tests
  getBranches: getBranchController,
  createBranch: addBranchController,
};
